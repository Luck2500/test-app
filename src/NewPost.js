    ...
    <div className="newpost_content">
      ...
      <div className="newpost_creator">
        <input className="newpost_creator" type="text" id="creator_input"
          placeholder="จากโดย" onChange={(event) => setCreator(event.target.value)}
          value={creator} />
      </div>
      <div className="newpost_title">
        <input className="newpost_title" type="text" id="title_input"
          placeholder="ชื่อท่องเที่ยว" onChange={(event) => setTitle(event.target.value)}
          value={title} />
      </div>
      <div className="newpost_text">
        <textarea className="newpost_text" rows='10' id="content_input"
          placeholder="ข้อมูลเพิ่มเติม" onChange={(event) => setText(event.target.value)}
          value={text} />
      </div>
      <div>
        <button className="create_button" onClick={handleCreate}>เพิ่มรายการ</button>
      </div>
    </div>
  )
